...
module.exports = function(config) {
    ...
    var convos_db = new Store(config.path + '/convos', {saveId: 'id'});
    ...
    var storage = {
        convos: {
            get: function(user, event, cb) {
                var result;
                var err;
                convos_db.get(user, function (error, data) {
                    result = data[event]
                    err = result ? null : error 
                    cb(err, result)
                });
            },
            save: function(data, cb) {
                var data = {
                    id: data.context.user,
                    event: data.context.event,
                    thread: data.thread 
                }

                var id = data.id;
                console.log('convos save', id, data)
                convos_db.save(id, data, cb);
            },
            delete: function(team_id, cb) {
                ...
                convos_db.save(data.id, data, cb);
            },
            all: function(cb) {
                convos_db.all(objectsToList(cb));
            }
        },
        teams: {
