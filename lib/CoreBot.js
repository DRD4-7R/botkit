...
function Botkit(configuration) {
    ...
    botkit.receiveMessage = function(bot, message) {
        ...
        botkit.middleware.receive.run(bot, message, function(err, bot, message) {
            if (err) {
                ...
                botkit.debug('RECEIVED MESSAGE', message.type);
    ...
    function Conversation(task, message) {
        botkit.debug('new Conversation', message.type);
        ...
        this.addMessage = function(message, thread) {
            botkit.debug('addMessage', thread);
        ...
        this.gotoThread = function(thread) {
            ...
            var makeChange = function() {
                ...
                botkit.debug('============================');
                botkit.debug('gotoThread for convo', that.context.event, 'and thread', that.thread, 'for user', that.context.user);
                botkit.debug('============================');

                botkit.storage.convos.save(that);
            };
    ...
    function Task(bot, message, botkit) {
        ...
        this.conversationEnded = function(convo) {
            ...
            botkit.middleware.conversationEnd.run(this.bot, convo, function(err, bot, convo) {
                ...
                botkit.debug('============================');
                botkit.debug('end convo', convo.context.event, 'for user', convo.context.user);
                botkit.debug('============================');

                botkit.storage.convos.delete(convo.context.user, convo.context.event);
    ...
    botkit.storage = {
        convos: {
            get: function(user, event, cb) {
                cb(null, botkit.memory_store.convos[user][event]);
            },
            save: function(convo, cb) {
                if (convo.context.user && convo.context.event) {
                    botkit.memory_store.convos[convo.context.user][convo.context.event] = convo.thread;

                    cb(null, convo.context.user, convo.context.event);
                } else {
                    cb('No user or event specified');
                }
            },
            delete: function(user, event, cb) {
                delete(botkit.memory_store.convos[user][event]);
                cb();
            },
            all: function(cb) {
                cb(null, botkit.memory_store.convos);
            }
        },
        teams: {
    ...
    if (configuration.storage) {
        if (
            configuration.storage.convos &&
            configuration.storage.convos.get &&
            configuration.storage.convos.save &&
